...
export const generatePlanPDF = async (planData) => {
    try {
        ...
        const { title, description, planType, duration, createdAt, overview, weeklySchedule } = planData;
        ...
        doc.setProperties({
            ...
            keywords: isNutrition ? "nutrition, diet, meal plan" : "training, workout, fitness",
        });

        // Helper function for text wrapping
        const addWrappedText = (text, x, y, maxWidth, lineHeight, fontSize = 11, font = "helvetica", style = "normal") => {
            if (!text) return y;
        ...
        doc.setFont("helvetica", "bold");
        doc.setFontSize(14);
        doc.text(isNutrition ? "NUTRITION PLAN" : "TRAINING PLAN", pageWidth / 2, 55, { align: "center" });
        ...
        doc.setFont("helvetica", "bold");
        doc.text(isNutrition ? "Nutrition" : "Training", pageWidth - 120, footerY + 10);
        doc.setFont("helvetica", "normal");
        ...
        doc.text("© Antique Body. All rights reserved.", pageWidth / 2, pageHeight - 7, { align: "center" });
        ...
        doc.setFontSize(11);
        doc.setTextColor(60, 60, 60);
        yPos = addWrappedText(overview.targetAudience, 20, yPos, pageWidth - 40, 5) + 15;
        ...
        if (weeklySchedule) {
            ...
            for (const day of days) {
                if (weeklySchedule[day]) {
                    ...
                    const dayTitle = daySchedule.title || `${day.charAt(0).toUpperCase() + day.slice(1)}`;
                    ...
                    doc.setDrawColor(isNutrition ? 34 : 30, isNutrition ? 197 : 64, isNutrition ? 94 : 158);
                    doc.setLineWidth(0.3);
        ...
        for (let i = 1; i <= pageCount; i++) {
            ...
            if (i > 1) {
                ...
                doc.setTextColor(100, 100, 100);
                doc.setFontSize(8);
                doc.text(`Page ${i - 1} of ${pageCount - 1}`, pageWidth - 20, pageHeight - 7, { align: "right" });
                ...
                doc.text(`${isNutrition ? "Nutrition" : "Training"} Plan: ${title}`, 20, pageHeight - 7);
            }
        }
