...
export const useTrainingPlanManager = (plan, setPlan) => {
        ...
        setPlan((prev) => ({
          ...
          schedule: prev.schedule.map((day, idx) =>
            idx === dayIndex
              ? {
                  ...day,
                  exercises: [...day.exercises, ...normalizedExercises],
                }
              : day
          ),
        }));
  ...
  const computed = useMemo(
    () => ({
       ...
       */
      totalDuration:
        plan.schedule?.reduce((total, day) => total + (day.duration || 0), 0) ||
        0,
      ...
      completionStats: {
        completedDays:
          plan.schedule?.filter((day) => day.workoutStatus === "completed")
            .length || 0,
        ...
        plan.schedule?.flatMap(
          (day, dayIdx) =>
            day.exercises
              ...
              .filter(
                (item) => item.exercise.repsUnit === REPS_UNITS.SECONDS
              ) || []
        ) || [],
    }),
    [plan]
  ...
  // Validation methods
  // Define validation functions first to avoid circular references
  ...
  const validateExercise = useCallback(
    (dayIndex, exerciseIndex) => {
      ...
      if (!exercise.reps || exercise.reps < 1)
        errors.push("Reps/seconds must be at least 1");
  ...
  const validatePlan = useCallback(() => {
    ...
    plan.schedule?.forEach((day, dayIdx) => {
      const dayValidation = validateDay(dayIdx);
      ...
      if (!day.isRestDay && day.exercises) {
        day.exercises.forEach((exercise, exerciseIdx) => {
          const exerciseValidation = validateExercise(dayIdx, exerciseIdx);
          if (!exerciseValidation.isValid) {
            errors.push(
              `Day ${dayIdx + 1}, Exercise ${
                exerciseIdx + 1
              }: ${exerciseValidation.errors.join(", ")}`
            );
          }
        });
      }
    ...
    return { isValid: errors.length === 0, errors };
  }, [plan, validateDay, validateExercise]);

  // Assign validation functions to the validation object
  const validation = useMemo(
    ...
    [validateDay, validateExercise, validatePlan]
  );
