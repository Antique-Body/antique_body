  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-[#111] to-[#0a0a0a] border border-[#333] rounded-2xl p-6">
        ...
        <div className="space-y-6">
          <div>
            <label className="block text-sm text-gray-400 mb-3">Difficulty Level</label>
            <div className="grid grid-cols-4 gap-4">
              {["beginner", "intermediate", "advanced", "expert"].map((level) => (
                <button
                  key={level}
                  onClick={() => handleSettingChange("workoutPreferences", "difficulty", level)}
          ...
          <div>
            <label className="block text-sm text-gray-400 mb-3">Focus Areas</label>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {["strength", "cardio", "flexibility", "endurance", "power", "balance"].map((area) => (
                <button
                  key={area}
                  onClick={() => {
                    const newAreas = settings.workoutPreferences.focusAreas.includes(area)
                      ? settings.workoutPreferences.focusAreas.filter(a => a !== area)
                      : [...settings.workoutPreferences.focusAreas, area];
                    handleSettingChange("workoutPreferences", "focusAreas", newAreas);
                  }}
                  className={`px-4 py-3 rounded-xl text-sm font-medium capitalize transition-colors ${
          ...
          <div>
            <label className="block text-sm text-gray-400 mb-3">Workout Duration (minutes)</label>
            <input
              type="range"
              min="15"
              max="120"
              ...
              onChange={(e) => handleSettingChange("workoutPreferences", "duration", parseInt(e.target.value))}
              className="w-full h-2 bg-[#333] rounded-full appearance-none cursor-pointer"
            />
            <div className="flex justify-between text-sm mt-2">
              <span>15 min</span>
              <span className="text-[#FF6B00]">{settings.workoutPreferences.duration} min</span>
              <span>120 min</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between p-4 bg-[#222] rounded-xl">
            <div>
              ...
              <p className="text-sm text-gray-400">Do you have access to a gym?</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
                checked={settings.equipment.hasGymAccess}
                onChange={(e) => handleSettingChange("equipment", "hasGymAccess", e.target.checked)}
          ...
          <div>
            <label className="block text-sm text-gray-400 mb-3">Available Equipment</label>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {["dumbbells", "barbell", "bench", "resistance bands", "kettlebell", "pull-up bar"].map((item) => (
                <button
                  key={item}
                  onClick={() => {
                    const newEquipment = settings.equipment.availableEquipment.includes(item)
                      ? settings.equipment.availableEquipment.filter(e => e !== item)
                      : [...settings.equipment.availableEquipment, item];
                    handleSettingChange("equipment", "availableEquipment", newEquipment);
                  }}
                  className={`px-4 py-3 rounded-xl text-sm font-medium capitalize transition-colors ${
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <button
                  key={goal.id}
                  onClick={() => handleSettingChange("goals", "primary", goal.id)}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => handleSettingChange("goals", "target", {
      ...
      <div className="bg-gradient-to-br from-[#111] to-[#0a0a0a] border border-[#333] rounded-2xl p-6">
        ...
        <div className="space-y-6">
          <div>
            <label className="block text-sm text-gray-400 mb-3">Preferred Days</label>
            <div className="grid grid-cols-7 gap-2">
              {["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"].map((day) => (
                <button
                  key={day}
                  onClick={() => {
                    const newDays = settings.schedule.preferredDays.includes(day)
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              {["morning", "afternoon", "evening"].map((time) => (
                <button
                  key={time}
                  onClick={() => handleSettingChange("schedule", "preferredTime", time)}
                  className={`px-4 py-3 rounded-xl text-sm font-medium capitalize transition-colors ${
