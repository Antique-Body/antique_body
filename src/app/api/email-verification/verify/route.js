...
export async function GET(request) {
  try {
    ...
    if (!token || !email) {
      return new Response(JSON.stringify({ success: false, message: "Token and email are required" }), {
        status: 400,
    ...
    if (!user) {
      return new Response(JSON.stringify({ success: false, message: "Invalid verification token or email" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
...
export async function POST(request) {
  try {
    ...
    if (!email || !token) {
      return new Response(JSON.stringify({ success: false, message: "Email and token are required" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
