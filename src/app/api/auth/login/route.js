...
export async function POST(request) {
    try {
        ...
        if (isEmailLogin && isPhoneLogin) {
            return NextResponse.json({ error: "Please provide either email or phone number, not both" }, { status: 400 });
        }
        ...
        if (isEmailLogin) {
            ...
            if (!user.password) {
                return NextResponse.json(
                    {
                        error: "This account was created with phone number. Please use phone login.",
                    },
                    { status: 400 }
                );
            }
        ...
        if (isPhoneLogin) {
            if (!code) {
                return NextResponse.json({ error: "Verification code is required for phone login" }, { status: 400 });
            }
            ...
            if (!user) {
                return NextResponse.json({ error: "Invalid phone number or verification code" }, { status: 401 });
            }

            // Verify code
            const isCodeValid = await userService.verifyPhoneCode(formattedPhone, code);
            const isCodeValid = await userService.verifyPhoneCode(formattedPhone, code);
            if (!isCodeValid) {
                return NextResponse.json({ error: "Invalid phone number or verification code" }, { status: 401 });
            }
