...
const Gallery = ({ trainerData, setTrainerData }) => {
  ...
  const onGalleryChange = useCallback((newGalleryImages) => {
    setTrainerData((prev) => ({
      ...prev,
      trainerProfile: {
        ...prev.trainerProfile,
    ...
    if (validFiles.length > 0) {
      ...
      if (availableSlots > 0) {
        ...
        if (filesToUpload.length < validFiles.length) {
          errorsToShow.push(`You can upload a maximum of ${MAX_GALLERY_IMAGES} images. ${filesToUpload.length} images were added.`);
        }
      } else {
        errorsToShow.push(`Your gallery is full. No more images could be added.`);
      }
    }
  ...
  const displayedImages = activeTab === "featured" ? highlightedImages : galleryImages;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-yellow-500/20 to-amber-500/20 rounded-lg p-4 border border-yellow-500/30">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-yellow-500">Featured Images</h3>
            <p className="text-sm text-gray-300">
              Featured images appear prominently at the top of your public profile. They're the first images potential clients see, so choose your best work!
            </p>
          </div>
        </div>
      </div>
          ...
          {displayedImages.length > 0 ? (
              ...
              {activeTab === "all" && (
                <div 
                  className={`relative flex aspect-square w-full items-center justify-center rounded-lg border-2 border-dashed ${
                    isHovering ? 'border-primary bg-primary/10' : 'border-gray-600'
                ...
                >
                  ...
                  <div className="flex flex-col items-center gap-2 text-gray-400">
                    <div className={`rounded-full p-3 ${isHovering ? 'bg-primary/20' : 'bg-gray-800'} transition-colors duration-300`}>
                      <Icon 
                        icon="mdi:upload" 
                        width={30} 
                        className={`${isHovering ? 'text-primary animate-bounce' : ''} transition-all duration-300`} 
                      />
                    </div>
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <div className="bg-gray-800 rounded-full p-4 mb-4">
                <Icon icon="mdi:star-outline" width={40} className="text-yellow-500" />
              </div>
              <h3 className="text-lg font-medium mb-2">No Featured Images</h3>
              <p className="text-gray-400 max-w-md mb-4">
                Feature your best images to showcase them prominently on your public profile.
              </p>
              <Button 
                variant="outline" 
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <div className="bg-gray-800 rounded-full p-4 mb-4">
                <Icon icon="mdi:image-outline" width={40} className="text-gray-400" />
              </div>
              <h3 className="text-lg font-medium mb-2">No Images Yet</h3>
              <p className="text-gray-400 max-w-md mb-4">
                Upload images to showcase your training style and results. A complete gallery helps potential clients understand your work.
              </p>
              <div 
                className="relative cursor-pointer bg-primary/10 hover:bg-primary/20 rounded-lg px-4 py-2 transition-colors duration-200"
              >
                <input
                  type="file"
                  multiple
      ...
      {galleryImages.length > 0 && (
        <div className="bg-gray-800/50 rounded-lg p-4 border border-gray-700">
          ...
          <ul className="text-sm text-gray-300 space-y-2 ml-6 list-disc">
            ...
            <li>Include before/after transformations (with client permission)</li>
