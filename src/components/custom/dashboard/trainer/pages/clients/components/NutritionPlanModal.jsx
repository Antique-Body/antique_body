...
import { DetailsTabNutrition } from "../../plans/components/planPreviewModal/DetailsTabNutrition";
...
export const NutritionPlanModal = ({ plan, isOpen, onClose, days }) => {
  ...
  // Set first day as default when plan/days changes
  useEffect(() => {
    let firstDay = null;
      ...
      <div className="flex flex-col -mt-6 -mx-4 sm:-mx-5">
        ...
        <div className="relative h-48 sm:h-64 w-full overflow-hidden">
          {coverImage && coverImage !== "" ? (
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-green-900/40 to-green-700/20 flex items-center justify-center">
              <Icon
                icon="mdi:food-apple"
                className="text-green-400 w-20 h-20 opacity-50"
              />
            </div>
          )}
          ...
          <div className="absolute top-4 left-4 z-10">
            <span className="px-3 py-1.5 text-sm font-medium rounded-md flex items-center gap-2 bg-green-900/80 text-green-100">
              <Icon icon="heroicons:beaker-20-solid" className="w-4 h-4" />
              Nutrition Plan
            </span>
          </div>
