...
export const BodyMetricsStep = ({
  ...
  const calculateBMI = () => {
    if (formData.weight && formData.height.feet && formData.height.inches) {
      ...
      const heightM =
        (parseInt(formData.height.feet) * 12 +
          parseInt(formData.height.inches)) *
      ...
      <RegistrationStep title="Body Metrics">
        ...
        {calculateBMI() && (
          <div className="mt-4 rounded-lg border border-[#333] bg-[rgba(20,20,20,0.7)] p-3">
            <p className="text-sm">
              Your BMI:{" "}
              <span className="font-bold text-[#FF6B00]">{calculateBMI()}</span>
            </p>
          </div>
        )}
      </RegistrationStep>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {fitnessLevelOptions.map((option) => (
              <FormField
                ...
                onChange={() =>
                  onChange({
                    target: { name: "fitnessLevel", value: option.value },
                  })
                }
              />
            ))}
