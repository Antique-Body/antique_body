...
export const BodyMeasurementsVisual = ({ measurements, previousMeasurements = null, highlightedMeasurement = null }) => {
    ...
    const calculateChange = (current, previous, metric) => {
        ...
        const isPositive = metric === "waist" || metric === "hips" ? change < 0 : change > 0;
                            ...
                            {measurementDetails.map((item) => (
                                ...
                                >
                                    <div className="flex items-center">
                                        <div
                                            className={`w-3 h-3 rounded-full mr-2 ${isHighlighted(item.id) ? item.activeClass : item.colorClass}`}
                                        ></div>
                                    ...
                                    <div className="flex items-center">
                                        ...
                                        {item.change && (
                                            <span
                                                className={`ml-2 text-xs ${item.change.isPositive ? "text-green-500" : "text-red-500"}`}
                                            >
                                                {item.change.value > 0 ? "+" : ""}
                                                {item.change.value}
                                            </span>
                                ...
                                <div className="flex flex-col space-y-2">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span
                                                className={`text-sm ${improved > total / 2 ? "text-green-500" : "text-gray-400"}`}
                                            >
                        ...
                        <div className="relative aspect-[3/4] w-full max-w-md mx-auto">
                            ...
                            <div className="absolute w-full h-full">
                                {measurementDetails.map((item) => (
                                    ...
                                    >
                                        <div
                                            className={`h-0.5 ${item.lineWidth} ${isHighlighted(item.id) ? item.activeClass : item.colorClass} rounded-full ${isHighlighted(item.id) ? "h-1" : ""}`}
                                        ></div>
                                        <div className="flex justify-center mt-1">
                                            <span
                                                className={`text-xs font-medium px-2 py-0.5 rounded ${isHighlighted(item.id) ? `bg-${item.colorClass.replace("bg-", "")}/30` : `bg-${item.colorClass.replace("bg-", "")}/20`}`}
                                            >
                                                {item.label}
                                            </span>
                                        </div>
                            ...
                            {measurementDetails.map((item) => (
                                ...
                                >
                                    ...
                                    <div className="flex items-center">
                                        ...
                                        {item.change && (
                                            <span
                                                className={`ml-1 text-xs ${item.change.isPositive ? "text-green-500" : "text-red-500"}`}
                                            >
                                                {item.change.value > 0 ? "+" : ""}
                                                {item.change.value}
                                            </span>
