...
export const FoodImageAnalyzer = ({ onAddToMeal, dailyGoals, dailyMacros }) => {
  ...
  const { calories: consumedCalories = 0, protein: consumedProtein = 0, carbs: consumedCarbs = 0, fat: consumedFat = 0 } = dailyMacros || {};
  ...
  // Calculate macro percentages for the food
  const macroPercentages = analysis ? {
    protein: Math.round((analysis.proteins * 4) / (analysis.calories) * 100),
    carbs: Math.round((analysis.carbs * 4) / (analysis.calories) * 100),
    fat: Math.round((analysis.fats * 9) / (analysis.calories) * 100)
  ...
  const renderProgressBar = (consumed, goal, value, label, color) => {
    ...
    const percentOfGoal = Math.min(Math.round((total / goal) * 100), 100);
      ...
      <div className="mb-3">
        <div className="mb-1 flex items-center justify-between">
          ...
          <div className="text-sm">
            ...
            <span className="text-gray-400">/{goal} {label !== "Calories" ? "g" : ""}</span>
          </div>
        </div>
      ...
      {!previewUrl && !manualInput ? (
        <Card variant="dark" width="100%" maxWidth="none" className="p-6">
          <div className="flex flex-col items-center justify-center text-center">
            ...
            <p className="mb-6 text-base text-gray-300">Upload a food photo to automatically identify and calculate its nutritional content</p>
              ...
              <div className="mt-4 flex justify-between">
                {!analysis ? (
                  <>
                    ...
                    >
                      {loading ? (
                        ...
                        manualInput ? "Search Food" : "Analyze Food"
                      )}
                    </Button>
                  </>
                ) : (
                  <div className="w-full">
                    ...
                    <p className="mb-2 text-sm text-gray-400">{analysis.description || "No description available"}</p>
                    <div className="mb-4 flex items-center justify-between">
                      <div>
                        <p className="text-sm text-gray-400">
                          <span className="font-medium">Source:</span> {analysis.brandName || "Generic"}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-400">
                          <span className="font-medium">Serving Size:</span> {analysis.servingSize} {analysis.servingUnit}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              {!analysis ? (
                <div className="flex h-full flex-col items-center justify-center p-6 text-center">
                  {loading ? (
                    <div className="flex flex-col items-center">
                      ...
                      <p className="mt-2 text-sm text-gray-400">This may take a few moments while we identify the food and calculate its nutritional content.</p>
                    </div>
                  ) : (
                    <>
                      <p className="mb-4 text-gray-300">
                        Upload and analyze a food image to see detailed nutritional breakdown and how it fits into your daily goals
                      </p>
                ...
                <div className="flex h-full flex-col">
                  ...
                  <div className="mb-6">
                    ...
                    {renderProgressBar(consumedCalories, calorieGoal, adjustedAnalysis.calories, "Calories", "bg-[#FF6B00]")}
                    {renderProgressBar(consumedProtein, proteinGoal, adjustedAnalysis.proteins, "Protein", "bg-blue-500")}
                    {renderProgressBar(consumedCarbs, carbsGoal, adjustedAnalysis.carbs, "Carbs", "bg-green-500")}
