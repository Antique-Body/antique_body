        ...
        <div className="w-full">
            ...
            <div className="flex items-center justify-center mb-4">
                <div className="flex items-baseline gap-1">
                    ...
                    <span className={`font-bold text-white ${isSmall ? "text-xl" : "text-3xl"}`}>{Math.round(total)}</span>
                    <span className={`text-gray-400 ${labelSize}`}>calories</span>
                </div>
            </div>

            {/* Progress bar with gradient transitions */}
            <div className={`relative ${barHeight} rounded-full overflow-hidden bg-[#111] shadow-inner w-full mb-5`}>
                {/* Protein section */}
                <div
                ...
                <div
                    ...
                    style={{ left: `${adjustedProteinPercent}%`, width: `${adjustedCarbsPercent}%` }}
                ></div>

                {/* Fat section */}
                <div
                    className="h-full absolute bg-gradient-to-r from-yellow-600 to-yellow-400 transition-all duration-500"
                    style={{ left: `${adjustedProteinPercent + adjustedCarbsPercent}%`, width: `${adjustedFatPercent}%` }}
                ></div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
                ...
                >
                    <div className="flex flex-col items-center">
                        ...
                        <div className="flex items-center mb-1">
                            <span className={`${titleSize} font-medium text-blue-400`}>Protein</span>
                        </div>

                        {/* Value */}
                        <span className={`${valueSize} font-bold text-white mb-0.5`}>{Math.round(protein)}</span>

                        {/* Percentage badge */}
                        <div className="bg-blue-500/10 rounded-full px-2 py-0.5">
                            <span className={`${percentSize} text-blue-400 font-medium`}>{proteinPercent}%</span>
                        </div>
                    </div>
                </div>
                ...
                >
                    <div className="flex flex-col items-center">
                        {/* Title and percentage */}
                        <div className="flex items-center mb-1">
                            <span className={`${titleSize} font-medium text-green-400`}>Carbs</span>
                        </div>

                        {/* Value */}
                        <span className={`${valueSize} font-bold text-white mb-0.5`}>{Math.round(carbs)}</span>

                        {/* Percentage badge */}
                        <div className="bg-green-500/10 rounded-full px-2 py-0.5">
                            <span className={`${percentSize} text-green-400 font-medium`}>{carbsPercent}%</span>
                        </div>
                    </div>
                </div>
                ...
                >
                    <div className="flex flex-col items-center">
                        {/* Title and percentage */}
                        <div className="flex items-center mb-1">
                            <span className={`${titleSize} font-medium text-yellow-400`}>Fat</span>
                        </div>

                        {/* Value */}
                        <span className={`${valueSize} font-bold text-white mb-0.5`}>{Math.round(fat)}</span>

                        {/* Percentage badge */}
                        <div className="bg-yellow-500/10 rounded-full px-2 py-0.5">
                            <span className={`${percentSize} text-yellow-400 font-medium`}>{fatPercent}%</span>
                        </div>
                    </div>
                </div>
            </div>
