                ...
                >
                    ...
                    >
                        ...
                        <div className="relative">
                            ...
                            <div className="absolute bottom-0 left-0 right-0 p-6 flex flex-col md:flex-row items-start md:items-end gap-6">
                                ...
                                <div className="flex-1">
                                    <h2 className="text-2xl md:text-3xl font-bold text-white">{trainer.name}</h2>
                                    <p className="text-[#FF6B00] font-medium">{trainer.specialty}</p>

                                    <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-2">
                                        <div className="flex items-center text-zinc-300">
                                            <Icon icon="mdi:map-marker" className="h-5 w-5 text-[#FF6B00] mr-1" />
                                            {trainer.location}
                                        </div>

                                        <div className="flex items-center">
                                            <RatingStars rating={trainer.rating} size={16} className="mr-1" />
                                            <span className="text-white font-medium">{trainer.rating.toFixed(1)}</span>
                                            <span className="text-zinc-400 ml-1">({trainer.reviewCount} reviews)</span>
                                        </div>

                                        <div className="text-zinc-300">
                                            <span className="text-[#FF6B00] font-medium">${trainer.price}</span>/session
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="md:hidden flex gap-2 px-4 mt-20 mb-4">
                            <Button variant="outline" size="small" className="flex-1 flex items-center justify-center gap-1">
                                <Icon icon="mdi:message" className="h-4 w-4" />
                                Message
                            </Button>
