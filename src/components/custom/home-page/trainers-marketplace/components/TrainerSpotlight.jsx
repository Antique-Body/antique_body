        ...
        >
            ...
            <div className="relative aspect-[2.5/1] w-full overflow-hidden">
                <AnimatePresence mode="wait">
                    ...
                    >
                        <Image src={activeTrainer.image} alt={activeTrainer.name} fill className="object-cover" priority />
                                ...
                                <AnimatePresence mode="wait">
                                    ...
                                    >
                                        <div className="inline-flex items-center px-3 py-1 bg-[#FF6B00]/20 border border-[#FF6B00]/40 rounded-full mb-4">
                                            <Icon icon="mdi:star" className="w-4 h-4 text-[#FF6B00] mr-1" />
                                            <span className="text-xs font-medium text-[#FF6B00]">Featured Trainer</span>
                                        </div>
                                        ...
                                        <p className="text-lg text-zinc-300 font-medium mb-1">{activeTrainer.specialty}</p>

                                        <div className="flex items-center text-zinc-300 mb-4">
                                            <Icon icon="mdi:map-marker" className="w-5 h-5 text-[#FF6B00] mr-1" />
                                            <span>{activeTrainer.location}</span>
                                            <span className="mx-2 text-zinc-500">•</span>
                                            <RatingStars rating={activeTrainer.rating} size={16} className="mr-1" />
                                            <span className="text-white font-medium">{activeTrainer.rating.toFixed(1)}</span>
                                            <span className="text-zinc-400 ml-1">({activeTrainer.reviewCount})</span>
                                        </div>

                                        <p className="text-zinc-300 mb-6 max-w-md">{activeTrainer.bio}</p>
                    ...
                    {trainers.map((_, index) => (
                        <button
                            ...
                            className={`w-2.5 h-2.5 rounded-full transition-all ${
                                index === activeIndex ? "bg-[#FF6B00] w-6" : "bg-zinc-600 hover:bg-zinc-500"
                            }`}
