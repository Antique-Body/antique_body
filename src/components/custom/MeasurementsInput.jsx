...
import { Button, ProgressBar, FormField } from "@components/common";
...
const BMI_CATEGORIES = {
  UNDERWEIGHT: {
    max: 18.5,
    label: "training_setup.measurements.bmi_categories.underweight",
    color: "text-blue-400",
  ...
  NORMAL: {
    max: 25,
    label: "training_setup.measurements.bmi_categories.normal",
  ...
  OVERWEIGHT: {
    max: 30,
    label: "training_setup.measurements.bmi_categories.overweight",
  ...
  OBESE: {
    max: Infinity,
    label: "training_setup.measurements.bmi_categories.obese",
    color: "text-red-400",
...
const calculateBmi = (weight, height, unitSystem) => {
  const weightInKg =
    unitSystem === UNIT_SYSTEMS.METRIC
      ? weight
      : weight * CONVERSION_FACTORS.WEIGHT.IMPERIAL_TO_METRIC;
  ...
  const heightInM =
    unitSystem === UNIT_SYSTEMS.METRIC ? height / 100 : (height * 30.48) / 100; // 1 foot = 30.48 cm
...
export const MeasurementsInput = ({ onSelect }) => {
      ...
      if (system === UNIT_SYSTEMS.METRIC) {
        ...
        const weightInKg = measurements.weight
          ? parseFloat(measurements.weight) *
            CONVERSION_FACTORS.WEIGHT.IMPERIAL_TO_METRIC
          : "";
        ...
        const weightInLbs = measurements.weight
          ? parseFloat(measurements.weight) *
            CONVERSION_FACTORS.WEIGHT.METRIC_TO_IMPERIAL
          : "";
  ...
  useEffect(() => {
    ...
    if (unitSystem === UNIT_SYSTEMS.METRIC) {
      ...
      const validFeet =
        !isNaN(feet) &&
        feet >= Math.floor(constraints.height.min) &&
      ...
      const withinRange =
        totalHeightInFeet >= constraints.height.min &&
    ...
    if (unitSystem === UNIT_SYSTEMS.METRIC) {
      ...
      if (touched.height && touched.inches && hasValidHeight === false) {
        newErrors.height = t(
          "training_setup.measurements.total_height_validation",
          {
            min: constraints.height.min,
            max: constraints.height.max,
          }
  ...
  const renderBmiDisplay = () => {
    ...
    return (
      <div className="mt-6 p-4  rounded-lg border border-[#222]">
        <h3 className="text-lg font-semibold mb-2">
          {t("training_setup.measurements.your_bmi")}
        </h3>
        ...
        <div className="flex justify-between text-xs text-gray-400">
          <span>
            {t("training_setup.measurements.bmi_categories.underweight")}
          </span>
          <span>{t("training_setup.measurements.bmi_categories.normal")}</span>
          <span>
            {t("training_setup.measurements.bmi_categories.overweight")}
          </span>
    ...
    <div className="w-full space-y-6">
      <div className="flex justify-center gap-4 mb-6">
        ...
        <Button
          ...
          variant={
            unitSystem === UNIT_SYSTEMS.IMPERIAL ? "primary" : "secondary"
          }
          className="min-w-[140px]">
      ...
      <Card width="100%" topBorderColor={false} borderTop={false}>
        ...
        {unitSystem === UNIT_SYSTEMS.METRIC ? (
          ...
          <FormField
            ...
            type="number"
            placeholder={t(
              "training_setup.measurements.height_placeholder_metric"
            )}
