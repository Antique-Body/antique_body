...
const calculateBmi = (weight, height, unitSystem) => {
    ...
    const heightInM = unitSystem === UNIT_SYSTEMS.METRIC ? height / 100 : (height * 30.48) / 100; // 1 foot = 30.48 cm
...
export const MeasurementsInput = ({ onSelect }) => {
    ...
    const constraints = useMemo(
        () => (unitSystem === UNIT_SYSTEMS.METRIC ? CONSTRAINTS.METRIC : CONSTRAINTS.IMPERIAL),
        [unitSystem]
    );
    ...
    useEffect(() => {
        ...
        if (unitSystem === UNIT_SYSTEMS.METRIC) {
            ...
            const validFeet =
                !isNaN(feet) && feet >= Math.floor(constraints.height.min) && feet <= Math.floor(constraints.height.max);
            ...
            const withinRange = totalHeightInFeet >= constraints.height.min && totalHeightInFeet <= constraints.height.max;
        ...
        return (
            <div className="mt-6 p-4 bg-[#0a0a0a] rounded-lg border border-[#222]">
                ...
                <div className="flex items-center justify-between mb-3">
                    ...
                    <div className={`text-lg font-medium ${bmiCategory.color}`}>{t(bmiCategory.label)}</div>
                </div>
